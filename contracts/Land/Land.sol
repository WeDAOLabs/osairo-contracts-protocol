...
contract Land is  ERC721, ERC721URIStorage, RandomNumber, AccessControl, VRFConsumerBaseV2{
    ...
    constructor(string memory name,string memory symbol) ERC721("LAND","LAND"){
    ...
    function mint(uint256 num) public payable {
        ...
        require(msg.value >= MINT_FEE * (101) ** num / 100**num, "Insufficient funds");
        require(tokenId + num <= MAX_NUM, "Exceed max supply");

        for(uint i = 0; i < num; i++) {
            _safeMint(msg.sender, tokenId);
            MINT_FEE = MINT_FEE * 101 / 100;
            emit MintUrl(tokenId, string(abi.encodePacked(BASE_URL, Strings.toString(tokenId))));
            tokenId++;
        }
    }
    //获取URL
