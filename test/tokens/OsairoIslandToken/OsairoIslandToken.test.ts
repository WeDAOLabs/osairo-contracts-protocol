...
describe("OsairoIslandToken", function () {
  let contract: Contract;

  beforeEach(async function () {
    const Token = await ethers.getContractFactory("OsairoIslandToken");
    contract = await upgrades.deployProxy(Token);
    await contract.deployed();
  });
  it("OsairoIslandToken test", async () => {
    expect(contract).to.be.instanceOf(Contract);
    expect(await contract.name()).to.equal("OsairoIslandToken");
    expect(await contract.symbol()).to.equal("OIT");
  });

  it("OsairoIslandToken: shoule get Role to Mint", async () => {
